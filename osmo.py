...
def doctor_recommendations(results: list[tuple[str, str, str]], project_dir: Path) -> list[str]:
    ...
    for name, status, message in results:
        ...
        if name == "tool:python3":
            ...
            add(f"Recreate project entrypoint: osmo wizard \"{project_dir}\"")
...
def cleanup_cmd(
    ...
    if interactive:
        ...
        remove_skill_manager_backups = click.confirm("Remove skill-manager backup archives (.pi/_skill-manager-backups)?", default=True)
    ...
    if remove_skill_manager_backups:
        _record_or_apply(devkit_backups, True)
