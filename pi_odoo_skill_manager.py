...
export ODOO_SKILL_MANAGER_ROOT=\"$(pwd)\"
...
export PATH=\"$ODOO_SKILL_MANAGER_ROOT/commands:$PATH\"
"""
...
<!-- managed-by: skill-manager installer -->

This file is generated by `pi-odoo-skill-manager.py`.

## Shared Skill Manager Links

- Skills: `.pi/skills/shared-skill-manager`
- Dev command: `.pi/skill-manager`

## Quick Commands

```bash
./.pi/skill-manager --help
./.pi/skill-manager up
./.pi/skill-manager db
./.pi/skill-manager shell
```
...
./pi-odoo-skill-manager.py wizard /path/to/odoo-project --with-browser-tools --yes
```
...
def _skill_manager_skills_backup_root(project_dir: Path) -> Path:
    return project_dir / ".pi" / "_skill-manager-backups" / "skills"
...
def sanitize_project_skills(project_dir: Path, managed_skills: set[str]) -> list[str]:
    ...
    old_backup_root = skills_root / "_disabled-by-skill-manager"
    backup_root = _skill_manager_skills_backup_root(project_dir)
...
def scan_content_hygiene(root: Path) -> tuple[str, str, str]:
    ...
    for path in root.rglob("*"):
        ...
        if path.name == ".envrc.local":
            continue
        if path.name == "pi-odoo-skill-manager.py":
            continue
...
def doctor_recommendations(results: list[tuple[str, str, str]], project_dir: Path) -> list[str]:
    ...
    for name, status, message in results:
        ...
        elif name == "skills:shared-skill-manager":
            add(f"Run setup: ./pi-odoo-skill-manager.py wizard \"{project_dir}\"")
        elif name == "link:.pi/skill-manager":
            add(f"Recreate project entrypoint: ./pi-odoo-skill-manager.py wizard \"{project_dir}\"")
        elif name == "pi:legacy-tools":
            add(f"Remove legacy artifacts: ./pi-odoo-skill-manager.py cleanup \"{project_dir}\" --all")
        elif name == "skills:collisions":
            add(f"Clean collisions: ./pi-odoo-skill-manager.py cleanup \"{project_dir}\" --all, then rerun setup.")
        elif name == "skills:invalid-artifacts":
            add(f"Archive invalid artifacts via setup: ./pi-odoo-skill-manager.py wizard \"{project_dir}\"")
...
def collect_skill_statuses(root: Path, project_dir: Path) -> list[SkillStatus]:
    ...
    enabled = current_enabled_names(project_dir / ".pi" / "skills" / "shared-skill-manager")
...
def enable_skill_for_project(root: Path, project_dir: Path, name: str) -> list[str]:
    ...
    dst = project_dir / ".pi" / "skills" / "shared-skill-manager" / name
...
def setup_project_quick(root: Path, project_dir: Path) -> list[str]:
    ...
    shared_skills_dir = project_dir / ".pi" / "skills" / "shared-skill-manager"
    ...
    local_skill_manager_link = project_dir / ".pi" / "skill-manager"
    target = root / "pi-odoo-skill-manager.py"
    if local_skill_manager_link.exists() or local_skill_manager_link.is_symlink():
        if not (local_skill_manager_link.is_symlink() and local_skill_manager_link.resolve() == target.resolve()):
            local_skill_manager_link.unlink()
            local_skill_manager_link.symlink_to(target)
            messages.append(f"Updated project entrypoint: {local_skill_manager_link}")
    else:
        local_skill_manager_link.symlink_to(target)
        messages.append(f"Created project entrypoint: {local_skill_manager_link}")
...
def cleanup_project_all(project_dir: Path, remove_local_exclude: bool = False) -> list[str]:
    ...
    legacy_skill_backup = project_dir / ".pi" / "skills" / "_disabled-by-skill-manager"
    devkit_backups = project_dir / ".pi" / "_skill-manager-backups"
    ...
    if local_skill_manager.exists() or local_skill_manager.is_symlink():
...
def run_tui(root: Path, project_dir: Path) -> None:
    def _app(stdscr: curses.window) -> None:
        ...
        while True:
            ...
            if h < 18 or w < 72:
                stdscr.addstr(1, 2, "pi-odoo-skill-manager")
            ...
            if key in (ord("\n"), curses.KEY_ENTER, ord(" ")):
                ...
                curses.endwin()
                try:
                    subprocess.call([sys.executable, str(root / "pi-odoo-skill-manager.py"), "doctor", str(project_dir)])
...
def wizard(
    ...
    if interactive:
        ...
        click.echo("This setup will create a project command at .pi/skill-manager.")
    ...
    if interactive:
        ...
        click.echo("\n  Project command")
        click.echo("    • Creates .pi/skill-manager (symlink to this devkit CLI)")
    ...
    target = root / "pi-odoo-skill-manager.py"
    if local_skill_manager_link.exists() or local_skill_manager_link.is_symlink():
        if local_skill_manager_link.is_symlink() and local_skill_manager_link.resolve() == target.resolve():
            pass
        else:
            local_skill_manager_link.unlink()
            local_skill_manager_link.symlink_to(target)
    ...
    click.echo("\n[project links]")
    click.echo(f"- Convenience link: {local_skill_manager_link} -> {local_skill_manager_link.resolve()}")
...
def run_doctor_checks(root: Path, project_dir: Path) -> tuple[list[tuple[str, str, str]], int, int]:
    ...
    if not skills_dir.exists():
        results.append(("skills:shared-skill-manager", "FAIL", f"missing: {skills_dir}"))
    else:
        entries = list(skills_dir.iterdir()) if skills_dir.is_dir() else []
        results.append(("skills:shared-skill-manager", "PASS", f"{len(entries)} entries"))

    local_skill_manager = project_dir / ".pi" / "skill-manager"
    if local_skill_manager.is_symlink():
        results.append(("link:.pi/skill-manager", "PASS", f"ok -> {local_skill_manager.resolve()}"))
    elif local_skill_manager.exists():
        results.append(("link:.pi/skill-manager", "WARN", "exists but not a symlink"))
    else:
        results.append(("link:.pi/skill-manager", "WARN", "missing"))
    ...
    if root_skills.exists():
        ...
        invalid = []
        for name in ["README.md", "_template", "_disabled-by-skill-manager"]:
            p = root_skills / name
            if p.exists() or p.is_symlink():
                invalid.append(name)
        if invalid:
...
def cleanup_cmd(
    ...
    remove_skills = True
    remove_skill_manager_link = True
    remove_notes = True
    remove_legacy_tools = True
    remove_skill_manager_backups = True

    if interactive:
        remove_skills = click.confirm("Remove installed shared skills from project (.pi/skills/shared-skill-manager)?", default=True)
        remove_skill_manager_link = click.confirm("Remove project skill-manager entrypoint (.pi/skill-manager)?", default=True)
        ...
        remove_skill_manager_backups = click.confirm("Remove skill-manager backup archives (.pi/_skill-manager-backups, .pi/_legacy-tools-disabled, old .pi/skills/_disabled-by-skill-manager)?", default=True)
...
def main(argv: list[str] | None = None) -> None:
    ...
    cli.main(args=args, prog_name="pi-odoo-skill-manager", standalone_mode=True)
